export default async function handler(req, res) {
    ...
    // Node 1: Resume context (RAG retrieval - embedded for lightweight deployment)
    const resumeContext = `
...
`.trim();
    ...
    // Node 2: System prompt with personality + RAG context
...
${resumeContext}`;
    ...
    // Node 3: Generate response via Groq LLM
    try {
        const groqMessages = [
            { role: 'system', content: systemPrompt },
            ...messages.slice(-10), // Keep last 10 messages for memory window
        ];
